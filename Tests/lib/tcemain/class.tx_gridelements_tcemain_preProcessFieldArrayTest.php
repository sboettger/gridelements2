...
class tx_gridelements_tcemain_preProcessFieldArrayTest extends Tx_Extbase_Tests_Unit_BaseTestCase {
	...
	public function testGetAvailableColumns() {
		...
		$layoutSetup
			...
			->will($this->returnValue($tcaColumns));
		$preProcess->injectLayoutSetup($layoutSetup);
		$beFunc = $this->getMock('wrapperForT3libBeFunc', array('getModTSconfig'));
		$beFunc
			->expects($this->once())
			->method('getModTSconfig')
			->will($this->returnValue(''));
		$preProcess->injectBeFunc(t3lib_div::makeInstance('wrapperForT3libBeFunc'));
		$preProcess->injectTceForms(t3lib_div::makeInstance('t3lib_TCEForms'));
		$result = $preProcess->getAvailableColumns($layout, $table, $id);
		$this->assertEquals($expectedTcaColumns, $result);*/
	}
}
